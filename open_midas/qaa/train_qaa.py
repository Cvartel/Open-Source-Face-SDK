...
def train_qaa(backbone_bnf, dataset, centroids, batch_size=512, device="cuda", save_path="./regressor"):
    """Train CatBoostRegressor to predict QAA and saves it on save_path

    arguments:
    backbone_bnf -- Backbone that return embedding and BN features (for example via BNFeatureExtractor)
    dataset -- torch dataset, must return image and label
    centroids -- centroids of each class. centroids[N] must be centroid of class with label N.
    """

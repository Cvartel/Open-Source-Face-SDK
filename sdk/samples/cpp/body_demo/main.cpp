...
void displayResultInWindow(Context& ioData,cv::Mat& image,std::string mode,bool output);
...
void demoBody(api::Service& service,const std::string& input_image_path,const std::string& mode ,const std::string& output){
	...
	///////////////////////////
	displayResultInWindow(ioData,image,mode,(output == "yes"));
}
...
void displayResultInWindow(Context& ioData,cv::Mat &image,std::string mode,bool output){
	for(const Context& obj : ioData["objects"])
	{
		...
		if(output) {std::cout << "Body detector:" << "X1:" << static_cast<int>(rectCtx[0].getDouble()*image.size[1]) << " Y1:" << static_cast<int>(rectCtx[1].getDouble()*image.size[0]) << " X2:" << static_cast<int>(rectCtx[2].getDouble()*image.size[1]) << " Y2:" << static_cast<int>(rectCtx[3].getDouble()*image.size[0]) << std::endl;}
		...
		if(mode == "pose"){
			...
			for(auto &bone: bone_map){
				...
				if(output) {std::cout << "Pose estimator:" << " X1:" << x1 << " Y1:" << y1 << " X2:" << x2 << " Y2:" << y2 << std::endl;}
			}
